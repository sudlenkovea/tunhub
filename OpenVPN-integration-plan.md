...
## Совместимость версий ядра OpenVPN

Актуальное community-ядро — **OpenVPN 2.6.x**. Оно **обратно совместимо**: клиент 2.6 подключается к серверам 2.5 / 2.4 и старше. OpenVPN 3 (`ovpncli`) тоже протокол-совместим с веткой 2.x. Единственный реальный нюанс — **согласование шифров (cipher negotiation)**:

- Дефолт data-ciphers в 2.6: `AES-256-GCM:AES-128-GCM:CHACHA20-POLY1305`.
- Старый сервер с фиксированным шифром **без** согласования (например `AES-256-CBC`, тем более `BF-CBC`, который в 2.6 выпилен из дефолта) → нужно либо добавить его в `--data-ciphers`, либо задать `--data-ciphers-fallback <cipher>`.
- `comp-lzo` / `compress` — устарели (уязвимость VORACLE): принимаем, но предупреждаем.

**Что из этого следует для нашей реализации:** `OVPNParser` должен сохранять `cipher` / `data-ciphers` из `.ovpn`, а супервайзер — для «старых» конфигов (где указан только `cipher X`) автоматически добавлять этот шифр в `data-ciphers` (как делает `--compat-mode 2.4`) или пробрасывать `--data-ciphers-fallback`. Тогда одно ядро 2.6 покроет и новые, и легаси-серверы. Практический вывод: **брать последнее 2.6 — можно, старые серверы не отвалятся**, надо лишь корректно проносить настройки шифров из профиля.
